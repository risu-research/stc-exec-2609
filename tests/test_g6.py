import unittest
from runner.rac import Action, ALLOW, DENY, UNRESOLVED, SELECTOR, EFFECT, compile_contract
from runner.rac_runtime import IndexedContract, SelectorAdapter, SelectorCall, EffectAdapter, EffectCall, descriptor
...
class G6Tests(unittest.TestCase):
    ...
    def test_receipt_cannot_cross_identity(self):
        ...
        self.assertTrue(self.i.permits(r,"update"))
        self.assertFalse(self.i.permits(r,"delete"))
    ...
    def test_descriptor_does_not_enumerate_domain(self):
        d=descriptor(self.i,"abc","resolved-action")
        raw=str(d)
        self.assertNotIn("update",raw)
        self.assertNotIn("delete",raw)
        self.assertEqual(d["contractId"],self.c.contract_id)
